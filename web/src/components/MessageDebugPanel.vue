  ...
  <div class="message-debug-panel">
    ...
    <div class="debug-panel-toolbar">
      <div class="toolbar-top-row">
        <div class="toolbar-title-group">
          ...
          <span v-if="auditsTruncated" class="item-count-badge" title="更早的审计记录未包含在当前响应中">
            仅最新 500 条审计
          </span>
        </div>
    ...
    <div class="timeline-container">
      <div v-if="filteredTimelineItems.length === 0" class="empty-timeline">
        ...
        <span class="empty-text">{{ emptyTimelineText }}</span>
      </div>
...
const emptyTimelineText = computed(() => {
  if (isAuditLoading.value) return '正在读取 Model/Tool 审计...'
  if (searchQuery.value) return '未找到匹配的消息'
  return '当前会话暂无消息数据'
})
