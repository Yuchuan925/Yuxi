...
def test_run_now_and_deleted_execution_history_keep_owner_scope(monkeypatch):
    calls = []
    ...
    async def fake_list_runs(*, job_id, user, **_kwargs):
        calls.append(("executions", job_id, user.uid))
        return [{"id": "execution-1", "thread_id": "thread-1"}]

    monkeypatch.setattr(router_module, "run_scheduled_job_now", fake_run_now)
    monkeypatch.setattr(router_module, "list_scheduled_job_runs", fake_list_runs)
    client = _client()

    assert client.post("/api/scheduled-tasks/job-1/run-now").json()["thread_id"] == "thread-1"
    assert client.get("/api/scheduled-tasks/job-1/executions").json()["executions"][0]["id"] == "execution-1"
    assert calls == [("run-now", "job-1", "user-1"), ("executions", "job-1", "user-1")]
