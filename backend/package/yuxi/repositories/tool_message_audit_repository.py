...
class ToolMessageAuditRepository:
    ...
    async def fail(
        ...
        *,
        ...
        return await self._finish(
            ...
            duration_ms=duration_ms,
            finished_sequence=finished_sequence,
            allow_failed_enrichment=allow_terminal_enrichment,
        )
    ...
    async def _finish(
        ...
        *,
        ...
        duration_ms: int | None,
        finished_sequence: int | None,
        allow_failed_enrichment: bool = False,
    ) -> Message:
        operation_id = tool_call_id.strip()
        if not operation_id:
            raise ValueError("Tool tool_call_id 不能为空")
        ...
        if message.execution_status == execution_status:
            if execution_status == "failed" and allow_failed_enrichment and not message.content:
                metadata["stream_error_message"] = metadata.get("error_message")
                metadata["error_message"] = error_message
