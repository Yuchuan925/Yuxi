...
- `worker-dev`：ARQ worker，执行已经派发的 AgentRun，并周期触发用户自建 Agent 定时任务；AgentRun 使用 attempt ownership 与 heartbeat 维护运行租约，定时任务使用 PostgreSQL 任务锁保证单次触发。
...
- 用户定时 Agent：PostgreSQL 保存用户自己的 Project、Agent、审批模式、cron、IANA 时区、提示词、下一次触发时间和执行记录；每次触发创建绑定原 Project 的新 Conversation，worker 锁定到期任务后复用统一 AgentRun Request/Run 链路，Redis/ARQ 只负责唤醒。
