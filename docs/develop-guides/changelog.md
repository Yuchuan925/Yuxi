...
- 新增通用管理员配置 Options 模块：系统运行时配置迁移到 PostgreSQL，API 与 worker 通过带版本失效的 Redis 短缓存共享最新值，Redis 故障时回源数据库；旧 `base.toml` 只补充缺失字段，不覆盖管理员值。OCR 作为首批消费者，敏感字段仅返回脱敏状态。LangGraph checkpoint 默认统一使用 PostgreSQL，并由进程级 manager 串行初始化；附件正式文件迁移到 MinIO，本地仅按需缓存，元数据与请求绑定统一以 PostgreSQL 为事实来源，Run 显式加载附件状态，不再依赖 checkpoint 双写。
