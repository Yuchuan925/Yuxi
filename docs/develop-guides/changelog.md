...
- 新增 OCR 引擎配置中心：使用 PostgreSQL `ocr_engine_configs` 统一管理 7 种 OCR 引擎的默认项、启停、自托管端点和凭证来源；凭证仅支持环境变量或数据库明文保存，云服务端点由系统管理。普通用户通过 `/api/system/ocr/options` 读取可用引擎，管理员编辑配置。每个 OCR 任务直接从 PostgreSQL 读取最新配置，不维护 OCR 配置缓存、通用识别参数或独立健康检查中心；文件只保存引擎选择和分块参数。
