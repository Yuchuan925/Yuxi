...
- 新增通用管理员配置 Options 模块：使用 PostgreSQL `config_options` 保存系统维护的名称、说明、受控表单参数和管理员 JSON 值，核心定义、校验、显式 `sensitive` 字段脱敏及“数据库非空值 → 环境变量”解析集中在 `config/options.py`；每个 `Option.get()` 均直接查询数据库，不建立进程内缓存，接口仅管理员可用。OCR 作为首批消费者，不再建立专用配置表或维护逐引擎启停状态；设置弹窗保留独立“OCR 配置”页，每个服务使用独立卡片，非编辑态通过禁用输入框展示数据库值、环境变量来源或敏感字段脱敏预览，点击编辑后可取消或保存，留空保存会清除数据库值并回退环境变量；数据库密钥只显示真实首尾字符的脱敏预览，环境变量只显示配置来源。设置页不集中陈列健康状态。附件添加、知识库解析和默认 OCR 设置统一复用 OCR Selector，选择器展开时实时检测全部方法健康状态，可用方法只显示标题和状态，不可用方法默认折叠并展示不可用原因，管理员可通过“去配置”直接打开 OCR 配置页；Selector 默认隐藏 `disable`，但后端配置接口仍允许保存该值。MinerU、PP-Structure 和 PaddleOCR 可使用数据库配置或环境变量，API Key / Token 允许明文入库但界面建议优先使用环境变量；DeepSeek OCR 固定复用 `siliconflow-cn` 模型供应商且不显示独立表单。普通用户仍通过 `/api/system/ocr/options` 获取所有代码支持的方法，文件只保存方法选择和分块参数。各 parser 现在自行声明 `service_name`、展示名和支持格式，避免 registry 重复维护元数据；生产解析统一通过 service 层 `parse_document()` 解析运行时配置后调用底层 parser，底层 `unified.py` 不再读取数据库或反向依赖 service。
