...
`yuxi.utils.paths.open_directory_fd` 是宿主进程中逐层打开目录的共享实现。函数从可信路径或调用方已经固定的目录 fd 开始，逐层打开已校验的单路径组件；`create=True` 时以现有宿主语义 `0o777` 创建缺失目录。函数返回由调用方关闭的 fd；传入目录 fd 时先复制，不接管原 fd。

共享函数拥有 fd 生命周期、目录创建和底层错误分类。Linux 可能把 symlink 与普通非目录组件都报告为 `ENOTDIR`，函数在父目录 fd 仍有效时以 no-follow `stat` 识别 symlink，并将其规范为 `OSError(ELOOP)`；真实非目录组件保留 `ENOTDIR`。Workdir、WorkspaceFilesystem 与 Skills 的薄 wrapper 将底层 errno 翻译为各边界已有的 `ValueError`、`PermissionError` 或原始 `NotADirectoryError`。

宿主侧创建权限保持 `0o777`。远端 Sandbox 脚本的 `0o755`、远端文件操作脚本与独立 provisioner 逻辑保持自包含，不导入 `yuxi`。
...
- 同时统一 `0o755` 与 `0o777`：拒绝。权限对齐是行为变化，需要单独判断运行用户与部署语义。

## 后果

- Workdir、WorkspaceFilesystem 和个人 Skill 复用同一套 fd/no-follow 遍历、创建与失败清理。
...
| 权限位和独立运行时脚本保持当前行为 | 重构顺带改变目录权限或镜像依赖 | 各调用点与独立脚本 | diff 与源码检查；Sandbox/Skills/Workspace 相关 unit 纳入 `187 passed` | `0o755/0o777` 未对齐，独立镜像未导入 `yuxi` | Passed |

旧能力不存在：宿主 `yuxi` 包内的 Workdir、WorkspaceFilesystem 和个人 Skill 不再保留独立的逐层创建/打开循环；各业务 wrapper 只保留 anchor 选择和异常翻译。远端执行脚本与独立 provisioner 明确排除。
