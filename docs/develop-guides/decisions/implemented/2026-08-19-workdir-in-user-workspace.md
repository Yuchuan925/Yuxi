...
API、worker 与 Sandbox 镜像使用不同进程 UID 访问同一 UserWorkspace。创建 Owner 在文件描述符仍打开时把新目录设为 `0777`、新文件设为 `0666`，不受进程 umask 影响；只处理本次新建对象，不递归改写既有目录树，也不把权限修复交给 provisioner。
...
| API、worker 与迁移器新建的 UserWorkspace 对象可由 Sandbox UID 写入 | umask 产生 root-owned `0755/0644`，Sandbox 只能读取而不能原子替换 | shared directory helper、WorkspaceFilesystem、Workspace service、v0.7.1 Workdir migration | restrictive-umask unit/integration + attachment runtime E2E | 权限更新失败清理 fd/部分对象；既有对象不被改写 | unit 通过；Linux runtime E2E 由 PR Runtime System Tests 验证 |
