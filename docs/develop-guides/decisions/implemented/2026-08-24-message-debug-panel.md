...
每条原始消息对应一行，折叠态只显示角色图标、角色和摘要。AI 摘要同时显示文本与 `tool_calls` 中的工具名称；独立 tool、system、resume 和未知类型消息不因聊天展示投影而丢失。展开态使用可折叠 JSON 树查看同一条消息的原始对象。
...
- `web/test/unit/messageDebug.test.js` 证明调试转换保持输入顺序、保留独立 tool/system 消息，并解析去重工具名称；删除 tool 映射或重新按聊天轮次过滤会使测试失败。
