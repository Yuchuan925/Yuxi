...
开发和生产 Compose 默认使用单 worker，`ARQ_MAX_JOBS=140`，其中 100 个槽位服务目标业务并发，40 个槽位留给健康检查、恢复、清理和短时峰值。API 与 worker Redis 池分别为 256；API/worker SQLAlchemy 池分别为 120+40；worker LangGraph 池为 120；PostgreSQL `max_connections=600`；Sandbox 并行删除数为 32。配置入口、完整核算方式和当前实测由 [Agent 并发容量](../../../advanced/agent-concurrency-capacity.md)维护。
...
API 与 worker 通过各自的 Redis 池配置吸收 SSE、事件发布、取消、ARQ、健康检查和缓存的短操作峰值。连接池按需建立，不能把 256 误读为常驻连接数，也不能按 Run 数一比一估算。
...
- PostgreSQL Schema 迁移从 business v2 升级到 v3；迁移器在真实 main 数据库执行成功，版本与 `prepared_at`、`first_output_at` 两列均由数据库回读确认，API/worker readiness 为 ready。
