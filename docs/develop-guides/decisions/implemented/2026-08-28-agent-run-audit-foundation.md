...
business schema 从 v1 相邻升级到 v2。唯一 storage migrator 先幂等增加 AgentRun trace 列，再发布 v2 版本；API 与 worker 继续只读校验精确版本。
...
- `backend/test/integration/services/test_schema_migration_version.py` 在隔离 PostgreSQL schema 中证明 v1→v2 DDL 幂等，且列提交后才发布版本。
