...
Yuxi 将上传文件先保存为原文件，再解析为 Markdown 并按知识库分块策略入库。管理员可在“设置 → OCR 配置”集中管理默认引擎、启停状态、自托管服务端点、凭证来源和识别参数；知识库上传或临时附件解析仍可逐次选择引擎，未显式选择时使用配置中心的默认项。
...
- 选择全局默认 OCR 引擎，并停用不允许新任务选择的引擎
- 配置 MinerU、PP-Structure 等自托管服务端点；官方云服务端点由系统管理
- 选择环境变量、数据库密钥或模型供应商作为凭证来源；页面和接口不会返回已保存的密钥原文
- 调整各引擎经过后端字段白名单校验的识别参数，并执行健康检查

MinerU Official 与 PaddleOCR 云引擎可以读取固定环境变量，也可以由管理员在页面填写密钥并加密保存到业务数据库；数据库模式要求所有 API 与 worker 进程共享持久化的 `OCR_CREDENTIAL_ENCRYPTION_KEY`，未单独配置时使用显式设置的 `JWT_SECRET_KEY`，不会使用开发环境临时密钥。DeepSeek OCR 固定复用 `siliconflow-cn` 模型供应商凭证，不允许切换来源。API 与 worker 会从 PostgreSQL 刷新仅存在于进程内存的解密凭证；Redis 快照、配置接口和文件处理快照均不包含密钥。
...
文件级 `processing_params` 会同时保存 `ocr_engine`、合并后的 `ocr_engine_config`、分块策略和 `chunk_parser_config`。OCR 参数优先级为“本次请求或文件快照 > 系统默认参数 > 解析器代码默认值”。连接端点和凭证不会写入文件快照；重新解析或入库时使用已保存的识别行为，便于复现历史处理结果。
