...
- 选择环境变量或数据库明文作为凭证来源；页面和接口不会返回已保存的密钥原文

MinerU Official、DeepSeek OCR 与 PaddleOCR 云引擎可以读取固定环境变量，也可以由管理员在页面填写密钥并保存到业务数据库。每个 OCR 任务开始时直接从 PostgreSQL 读取一次最新配置，不维护 Redis 或进程内配置副本；配置接口和文件处理快照不返回密钥。
...
文件级 `processing_params` 保存 `ocr_engine`、分块策略和 `chunk_parser_config`。OCR 的连接端点和凭证始终从数据库读取，不写入文件快照。
...
6. **解析配置**：文件只保存当次 `ocr_engine` 与分块参数快照；端点和凭证始终使用最新数据库配置
