...
- 普通请求先持久化到 PostgreSQL，再由 ARQ 投递给 worker；`ARQ_MAX_JOBS` 是单个 worker 的执行槽上限。
...
| `API_REDIS_MAX_CONNECTIONS` | 256 | API Redis 客户端池上限；不是预先建立 256 个连接 |
| `WORKER_REDIS_MAX_CONNECTIONS` | 256 | worker Redis 客户端池上限；取消轮询不独占连接 |
...
| `POSTGRES_MAX_CONNECTIONS` | 600 | 上述单 worker 客户端预算共 440，余下 160 给迁移、健康检查和运维 |
...
Redis 256 是经过 100 并发验证的进程级保护上限，并非取消协议的最低要求。移除每 Run Pub/Sub 后，本次 100 并发只观察到 36 个 Redis 客户端连接；暂时保留 256 可以吸收 SSE、事件发布、缓存和控制面峰值，也避免为了降低一个不预分配的上限引入新的配置档位。资源受限部署可以压低它，但必须重新跑目标并发并确认没有连接池耗尽或事件缺失。
...
2. 核算 PostgreSQL 最坏预算：所有 API SQLAlchemy 池、所有 worker SQLAlchemy 池和所有 worker LangGraph 池之和，必须低于 `POSTGRES_MAX_CONNECTIONS`，并给 migrator、健康检查、连接抖动和人工运维留余量。增加 worker 副本会按副本数放大 worker 两类池。
