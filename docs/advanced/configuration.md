...
Project、User Data、Skill source 与 Skill projection 使用各自的显式存储目录；数据库、Redis、sandbox 和 LangGraph checkpointer 后端仍属于启动期环境变量配置。显式选择 SQLite checkpointer 时，API 与 worker 必须把同一持久目录配置为 `YUXI_CHECKPOINT_DIR`；Compose 默认挂载 `/app/checkpoints`。运行中的已初始化组件不承诺这些配置热更新，修改后需要重启服务。
